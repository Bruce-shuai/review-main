`Typescript` 讲究**编译**二字。 所谓 `TS` 编译即将 `TS` 代码转变为对应的 `JS` 代码。 因为浏览器和`Node`环境不能直接支持`TS`代码。


### 静态类型检查

TS 提供了静态类型检查，能够让代码还未被运行的时候(构建的时候)，就可以检查出一些错误。




### tsc  
`tsc`的全称是`Typescript Compiler`。即将TS转换为JS的编译器。

### tsconfig.json文件
一个用来自定义**TS编译**相关规则的文件

该文件对应配置的介绍：https://www.typescriptlang.org/tsconfig
